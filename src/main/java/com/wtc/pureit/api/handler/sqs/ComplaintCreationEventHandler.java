...
@Slf4j
public class ComplaintCreationEventHandler implements RequestHandler<SQSEvent, String> {
    ...
    public ComplaintCreationEventHandler() {
            ...
            try {
                ...
                resp = retryTemplate.execute((RetryCallback<String, Exception>) retryContext -> {
                    ...
                    if (Objects.nonNull(complaintStatusResponse)) {
                        ...
                        String url = magentoCaller.getMagentoConfig().getUpdateComplaintIdToMagentoUrl()
                                .replaceAll("/id", "/" + complaint);

                        response = magentoCaller.call(url, requestBody, ApiMapper.POST, null);
                    }
                    return response;
                });
            } catch (Exception e) {
